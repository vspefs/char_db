...
requires std::is_object_v<T>
  ...
  public:
    constexpr
    non_propagating_cache () noexcept = default;
...
requires std::move_constructible<T> && std::is_object_v<T>
  ...
  public:
    ...
    operator= (movable_box const &other) noexcept (std::is_nothrow_copy_constructible_v<T>)
    ...
    constexpr movable_box &
    operator= (movable_box &&other) noexcept (std::is_nothrow_move_constructible_v<T>)
    {
      if (this != std::addressof (other))
        {
          if (other)
